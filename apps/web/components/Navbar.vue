    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center h-16">
        ...
        <nav class="hidden md:flex space-x-8">
          ...
          <NuxtLink v-if="auth.isAuthenticated" to="/dashboard"
            class="text-gray-700 dark:text-gray-300 hover:text-primary-600 font-medium">Mi Área</NuxtLink>
          <NuxtLink v-if="auth.isAdmin" to="/config"
            class="text-gray-700 dark:text-gray-300 hover:text-primary-600 font-medium">Admin</NuxtLink>

        </nav>

        <div class="flex items-center space-x-4">
          <button @click="toggleDarkMode"
            class="p-3 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
          ...
          <template v-if="auth.isAuthenticated">
            ...
            <button @click="auth.logout(); router.push('/')"
              class="px-6 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium transition">
              Logout
            </button>
          </template>
          <template v-else>
            <NuxtLink to="/login"
              class="px-6 py-2.5 bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-white rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 font-medium transition">
              Login
            </NuxtLink>
            <NuxtLink to="/register"
              class="px-6 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 font-medium transition">
              Register
            </NuxtLink>
          </template>
        </div>
